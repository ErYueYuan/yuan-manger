<template>
  <div>
    <div class="index-top4">
      <h3 class="ek-com-tit2 c6 f3">今日访客<span class="sp1 f-right c-p" @click="$router.push({path:'/visitornews'})">访客
<!--        <b class="fflag">5</b>-->
      </span></h3>
      <div style="height:200px;width:100%;" ref="myEchart"></div>
    </div>
    <div class="index-top4">
      <h3 class="ek-com-tit2 c6 f3">近7天热门访问产品</h3>
      <div class="  message-box4 mar7-1">
        <ul class="  ">
          <li class="b-radius3  f2 mar7-2 " v-for="(item, index) in productList" :key="index">
            <div class="t-left f2 c5">{{item.busName}}</div>
            <div class="f1 c4"><span>预览</span><span class="pad4 pad4-2">{{item.count || 0}}</span>{{item.viewTime}}{{item.viewer}}{{item.viewTime||item.viewer?'访问':''}}</div>
          </li>
        </ul>
        <van-empty v-if="productList.length === 0" description="暂无访问产品，快去分享吧" />
      </div>

    </div>
    <div class="index-top4">
      <h3 class="ek-com-tit2 c6 f3">近7天热门访问文章</h3>
      <div class="  message-box4 mar7-1">
        <ul class="  ">
          <li class="b-radius3 spa f2 mar7-2 " v-for="(item, index) in articleList" :key="index">
            <div class="t-left f2 c5">{{item.busName}}</div>
            <div class="f1 c4"><span>预览</span><span class="pad4 pad4-2">{{item.count || 0}}</span>{{item.viewTime}}{{item.viewer}}{{item.viewTime||item.viewer?'访问':''}}</div>
          </li>
        </ul>
        <van-empty v-if="articleList.length === 0" description="暂无访问文章，快去分享吧" />
      </div>

    </div>
  </div>
</template>

<script>
  import Vue from 'vue'
  import { mapGetters } from 'vuex'
  import * as tool from '@/common/Tool'
  import { api_statistics } from '@/api/index'
  import echarts from 'echarts'
  import { Empty } from 'vant'

  Vue.use(Empty)

  export default {
    name: "Visitor",
    data () {
      return {
        barData: {},
        productList: [],
        articleList: []
      }
    },
    computed: {
      ...mapGetters(['user']),
    },
    created () {
      this.getCustomerBar()
      this.getVisitTrailProduct()
      this.getVisitTrailArticle()
    },
    mounted () {
      this.initChart()
    },
    methods: {
      getVisitTrailProduct () {
        const _data = {
          offset: 0,
          limit: 3,
          agentCode: this.user.userInfo.agentcode || '',
          saasId: tool.app.saasId,
          eventCode: "productShare"
        }
        api_statistics.getHotVisitTrail(_data).then(data => {
          // if (data.status === tool.rtCode.status) {
            this.productList = data.result
          // } else {
          //   tool.toastMessage(data.message, 'error')
          // }
        })
      },
      getVisitTrailArticle () {
        const _data = {
          offset: 0,
          limit: 3,
          agentCode: this.user.userInfo.agentcode || '',
          saasId: tool.app.saasId,
          eventCode: "articleDetailShare"
        }
        api_statistics.getHotVisitTrail(_data).then(data => {
          // if (data.status === tool.rtCode.status) {
            this.articleList = data.result
          // } else {
          //   tool.toastMessage(data.message, 'error')
          // }
        })
      },
      getCustomerBar () {
        const _data = {
          saasId: tool.app.saasId,
          userId: this.user.userInfo.userId || '',
          //openId: 1
        }
        api_statistics.getCustomerTrail(_data).then(data => {
          if (data.status === tool.rtCode.status) {
            let result = data.result
           // console.log(result)
            if (result.length > 0) {
              let xAxisData = [], seriesData = []
              for (let item of result) {
                xAxisData.push(item.eventName)
                seriesData.push(item.count)
              }
              this.barData = {
                xAxisData: xAxisData,
                seriesData: seriesData
              }
              this.initChart()
            }
          }
        })
      },
      initChart () {
        let CW = document.body.offsetWidth || document.documentElement.clientWidth
        const chart = echarts.init(this.$refs.myEchart)
        chart.clear() //清除旧数据
        chart.resize({
          width: CW - 60,
        })
        chart.setOption({
          backgroundColor: '#f9fafd',
          color: ['#fe961a'],
          tooltip: {
            trigger: 'axis',
            axisPointer: {            // 坐标轴指示器，坐标轴触发有效
              type: 'shadow'        // 默认为直线，可选为：'line' | 'shadow'
            }
          },
          grid: {
            left: '3%',
            right: '4%',
            bottom: '3%',
            containLabel: true
          },
          textStyle: {
            color: '#666666',
            fontSize: 16
          },
          xAxis: [
            {
              type: 'category',
              data: this.barData.xAxisData, 
              axisTick: {
                show: false
              },
              axisLabel: {
                interval: 0,
                rotate: 35,
                margin: 10,
                color: '#666666',
                fontSize: 14
              },
              axisLine: {
                lineStyle: {
                  color: '#e1e2ec'
                }
              }
            }
          ],
          yAxis: [
            {
              type: 'value',
              axisLine: {
                lineStyle: {
                  color: '#e1e2ec'
                }
              },
              axisLabel: {
                color: '#999999',
                fontSize: 14
              },
              axisTick:{       // y轴刻度线
                "show":false
              },
              splitLine: {
                show: false
              },
              interval: 5 // y轴跨度
            }
          ],
          series: [
            {
              type: 'bar',
              barWidth: '50%',
              data: this.barData.seriesData, 
              label: { // 图形上的文本标签
                show: true,
                position: 'top'
              },
              itemStyle: {    // 图形的形状
                barBorderRadius: [5, 5, 0 ,0]
              }
            }
          ]
        })
      }
    }
  }
</script>

<style scoped>

</style>
