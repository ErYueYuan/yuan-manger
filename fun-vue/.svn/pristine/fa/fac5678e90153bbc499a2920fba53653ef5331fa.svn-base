  ...
  <div>
    <div v-show="!isGroupChatStatus">
      <div class="fun-top-panel bg7  ">
        ...
        <div class="chat-wrapper mar3-7 posi2 clearfix">
          ...
          <div class=" chatting-list ">
              ...
              <lazy-component>
                <div class="blk-cont-box mar6-2  posi2  border-t" v-for="(item, index) in conversation" :key="index" @click="goMessWindow(item)">
                  ...
                  <div class="right-box rb1 posi2 c-p" >
                    <h3 class="f2 c6 mar7-2">
                      <span class="mess w5 dblock">{{item.name}}</span><span style="width:30%;" class="dblock f-right f1 c4">{{item.modifydate}}</span>
                    </h3>
                    <div class="f2 c5 mess">
                      <span style="display: inline-block;width: 1px;" v-if="item.sendType === 'text'" v-html="item.content"></span>
                      <span style="display: inline-block;width: 1px;" v-if="item.sendType !== 'text'">{{item.sendType === 'news' ? '[链接]' + item.newsContent.title : (item.messageType === 'image' ? '[图片]' : item.content)}}</span>
                    </div>
                  </div>
                </div>
              </lazy-component>
            ...
            <van-tabs @click="handleTabsClick" title-active-color="#FD5147" color="#FD5147" v-if="messType==='qyWx'">
                  ...
                  <lazy-component>
                    <div class="blk-cont-box mar6-2  posi2  border-t" v-for="(item, index) in conversation" :key="index" @click="goMessWindow(item)">
                      ...
                      <div class="right-box rb1 posi2 c-p" >
                        ...
                        <div class="f2 c5 mess">
                          <span style="display: inline-block;width: 1px;" v-if="item.messageType === '10001'" v-html="item.messageContent"></span>
                          <span style="display: inline-block;width: 1px;" v-if="item.messageType !== '10001'">{{item.messageType === '10006' ? '[链接]' + JSON.parse(item.messageContent).title : (item.messageType === '10003' ? '[图片]' : item.messageContent)}}</span>
                        </div>
                      </div>
                    </div>
                  </lazy-component>
              ...
              <van-tab title="通讯录">
                <div class="main-top chat-group bg7  border-t">
    ...
    methods: {
      ...
      getOperatorManageInfo(ind) {
        api_mess.getOperatorManageQyWx({
          type: this.user.operatorManageInfo ? this.user.operatorManageInfo.id : '',
        }).then(res => {
          if (res.code == '200') {
      ...
      getMessList() {
        ...
        api_mess.getAllClientNewTalk(_data, {showLoadingFlag: 'N'}).then(data => {
          if (data.status === tool.rtCode.status) {
      ...
      goMessWindow(it) {
        ...
        let talkNum = this.messType === 'public' ? it.talkNum : (this.messType === 'qyWx' ? it.unreadNum : ''),
          ...
          parameter = this.messType === 'public' ? 'messType=public&oid=' + it.openid + '&aid=' + it.appId + '&cid=' + it.clientId : 'messType=qyWx&wid=' + it.operationWechatId + '&wctId=' + it.wechatId + '&' + wxId
        DataSource.set('talkNum', talkNum, 1)
        DataSource.set('messListInfo', it, 2)
