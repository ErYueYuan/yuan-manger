        ...
        <div class="blk-cont-box posi2">
          ...
          <div class="right-box rb2 posi2  c-p">
            ...
            <section
              ...
              :key="indexs"
              @click="$router.push({ name: 'dealdetails', params:{r:user.userInfo.agentcode,q:childitem.policyNo} })"
            >
              <div
                class="icon-num t-center"
    ...
    polstates(val) {
      ...
      api_sell.sysDict(_data).then(data => {
        if (data.status == tool.rtCode.status) {
          Object.keys(val).forEach(items => {
            val[items].forEach(childitem => {
              data.params.dicts.forEach(item => {
                if (item.value == childitem.invalidReason) {
                  this.name = item.name
                }
              })
            })
