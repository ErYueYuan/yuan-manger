<template>
  ...
  <div class="bg3">
    <div>
      <div class="main-top p-fix bg7 b-radius2" style="top: 0px;">
            ...
            <div class=" new-search ns clearfix">
              <section class="sear-c posi2 clearfix" @click="search()">
                <form action="" class="input-search-form" id="search_from">
                  <input type="search" disabled input autocomplete="off" placeholder="输入关键字搜索" ng-model="search.text"
                         class="f2 ">
                </form>
        ...
        <div class="sale-nav row mar0 pad6">
          <div class="col-6" @click="getCustomer()">
            <div class="sale-btn-l t-center"><b class="s-icon b1"></b>获取客户</div>
          </div>
          <div class="col-6" @click="createCustomer()">
            <div class="sale-btn-r t-center"><b class="s-icon b2"></b>自建客户</div>
          </div>
        </div>
      </div>
      ...
      <div class="main-container mar4-4 clearfix" style="margin-top: 146px;">
        <section class="bg7 b-radius2 ">
          <div class="clearfix bg7 f-flex">
            <div class="sel-p5 pro" :class="listStatus == 1 ? 'active' : ''" @click="chooseList(1)"><label class="f3-1"><b>待跟进</b></label>
            </div>
            <div class="sel-p5 pro" :class="listStatus == 2 ? 'active' : ''" @click="chooseList(2)"><label class="f3-1"><b>跟进中</b></label>
            </div>
            <div class="sel-p5 pro" :class="listStatus == 3 ? 'active' : ''" @click="chooseList(3)"><label class="f3-1"><b>计划跟进</b></label>
            </div>
            <div class="sel-p5 pro" :class="listStatus == 4 ? 'active' : ''" @click="chooseList(4)"><label class="f3-1"><b>待成交</b></label>
            </div>
          </div>
          <div class="bg7  border-t2 clearfix b-radius2 pad5-3">
            <div class="fresh box3">
              <div class=" sel-3 " :class="labelStatus == 1 ? 'on' : ''">
                <div class="abc" :class="labelStatus == 1 ? 'abc_selected' : ''" @click="selectClick('1')">
                  {{!labelTitle1 ? '客户类型' : labelTitle1}}
                </div>
              </div>
              <div class=" sel-3 " :class="labelStatus == 2 ? 'on' : ''">
                <div class="abc" :class="labelStatus == 2 ? 'abc_selected' : ''" @click="selectClick('2')">
                  {{!labelTitle2 ? '等级' : labelTitle2}}
                </div>
              </div>
              <!-- <div class=" sel-3 " :class="labelStatus == 3 ? 'on' : ''" v-show="listStatus == 1 || listStatus == 4">
                <div class="abc" :class="labelStatus == 3 ? 'abc_selected' : ''" @click="selectClick('3')">
                  {{!labelTitle3 ? '专案线索' : labelTitle3}}
                </div>
              </div> -->
              <div class=" sel-3 " :class="labelStatus == 4 ? 'on' : ''">
                <div class="abc" :class="labelStatus == 4 ? 'abc_selected' : ''" @click="selectClick('4')">
                  {{!labelTitle4 ? '排序' : labelTitle4}}
                </div>
              </div>
            </div>
        ...
        <section class="bg7 ">
          <div class="blk-cont-box posi2 border-b1 mar6-2" v-for="(item, index) in customerList" :key="index"
            ...
            <div class="img-box posi2">
              <img v-if="!!item.head" :src="item.head" alt="pic"/>
            ...
            <div class="right-box posi2  c-p">
              <h3 class="f2 c5 mar7-4"><span>{{item.name}}</span></h3>
              <div class="f2 c4 ">
              ...
              <div class=" f2 c4">
                <span>客户星级：</span>
                <span class="stars-box" v-if="item.customerStar == '1'"><i></i><i class="grey"></i><i
                  class="grey"></i><i class="grey"></i><i class="grey"></i></span>
                <span class="stars-box" v-if="item.customerStar == '2'"><i></i><i></i><i class="grey"></i><i
                  class="grey"></i><i class="grey"></i></span>
                <span class="stars-box" v-if="item.customerStar == '3'"><i></i><i></i><i></i><i class="grey"></i><i
                  class="grey"></i></span>
              ...
              <div class=" f2 c4" v-show="listStatus == 2 || listStatus == 3">
                <span>跟进结果：</span><span>{{item.followUpResult}}</span>
              </div>
              ...
              <div class="f1 pad6-4" v-if="!!item.activitySource"><b class="ek-btn-5 ">{{item.activitySource}}</b></div>
              <div class="f1 pad6-4" v-if="!!item.officialAccountName"><b class="ek-btn-5 ">{{item.officialAccountName}}</b></div>
            </div>
            <b class="flag">客户行销</b>
          </div>
        </section>

        <van-empty v-show="total===0" description="暂无客户"/>
        <div class="f2 pad888" v-if="customerList.length>=total && customerList.length>0">
          <div class="lzg-h-more posi2 t-center">
            <span class="c5 bg3 posi2 c222">没有更多啦</span>
          </div>
        </div>
      </div>
    </div>
      ...
      <div>
        ...
        <div class="mar3-2 ">
          <section class="pad5-5  bg7">
            <div class=" t-center">
              <van-stepper v-model.number="value" integer step="1" :max="surplusNum" :disable-input="isDisable" @change="changeNum"/>
            </div>
            <div class="f1 c5 t-center mar2">今日剩余可取数量{{maxNum}}人</div>
            <div class="pad6" @click="saveNum()">
              <button type="button" class="hx-bt3 bt2 active t-center" :class="maxNum == 0 ? 'cancel' : ''">取数</button>
            </div>
          </section>
        </div>
      </div>
    ...
    <div class="pop-box00" v-if="isIndepend">
      <div>
        <div class="pad5-9 bg7 b-radius1 bold flex-center">获取客户
          <img class="close_img" src="../../assets/img/close4.png" alt="" @click="closed('independ')">
        </div>
        <div class="mar3-2 max-h2">
          <section class="pad5-5  bg7 hide">
            ...
            <section class=" falg-bg">
              ...
              <div class="btn-box">
                <button class=" ek-btn-1 f1 ">取数</button>
              </div>
            </section>
          </section>
          <h3 class="pad5 c5 f1 hide">当前可用积分：1000分</h3>
          <section class="pad5-5  bg7" v-for="(item,index) in funAgentAccessDTOList" :key="index">
            <h3 class="f1 c6 l-h1 bold">{{item.customerStar}}星客户</h3>
            <div class="f1 c5">获取成本: <span class="c8">{{item.dataCost}}元/客户</span>
              <div class="f-right w4" @click="saveNum(item)">
                <button class=" ek-btn-1 f1 " :class="item.maxNum == 0 ? 'cancel' : ''">取数</button>
              </div>
            </div>
            <div class="pad6-1">
              <div class="flex-auto ">
                <span class="f1 c6 tit vh2">数量：</span>
                <van-stepper v-model.number="item.get_num" integer step="1" :max="item.surplusNum" :disable-input="isDisable" @change="changeNum($event,index)" class="dblock"/>
                <span class="f1 c4 vh2 mar5-1">(今日剩余可获取：{{item.maxNum}}人)</span>
              </div>
            </div>
            <div class="f1 c5 pad6-1 border-t">客户特色: {{item.dataAdvantage}}</div>
          </section>
        </div>
      </div>
    </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        value: 1,//取数的数量输入值
        maxNum: '',//最大取数量
        surplusNum: "",//目前分配的剩余可取数量
        isDisable: true,
        funAgentAccessDTOList: [],
      }
    },
    computed: {
      ...mapGetters(['user']),
    ...
    methods: {
      ...
      goDetail(item) {
        DataSource.set('customerDetailSource', 'sellList', 2);
        this.$router.push({
      ...
      selectContent(values, index) {
        this.isLabel = false
        if (values.type == 'funzg_batch_apply_mode') {
          if (values.name == '全部') {
      ...
      getCustomerList() {
        const _data = {
          ...
          batchApplyMode: this.labelValue1,//客户来源:1-分配客户;2-取数客户;3-自建客户
      ...
      getAgentAccess() {
        ...
        api_sell.getAgentAccess(_data).then(data => {
          if (data.status == tool.rtCode.status) {
            this.funAgentAccessDTOList = data.funAgentAccessDTOList.reverse();//倒序
      ...
      saveNum(item) {
        ...
        if (this.maxNum != 0) {
          const _data = {
            ...
            saasId: tool.app.saasId,
            userId: this.user.userInfo.userId,
            assignNum: item.get_num,
            customerStar: item.customerStar
          }
          ...
          // tool.toastMessage('今日剩余可取数量0人', 'error')
          // this.accessStatus = false;
        }
      },
