    ...
    <div class="main-top p-fix3 bg7 b-radius2">
      <div class="sale-nav row mar6-5 pad6">
        <div class="col-6 pad8">
          <div class="sale-btn-l t-center c-p"><b class="s-icon b5"></b>新建计划书</div>
        </div>
        <div class="col-6 pad8">
          <div class="sale-btn-r t-center c-p"><b class="s-icon b6"></b>对比计划书</div>
        </div>
      </div>
    </div>
      ...
      <section class=" b-radius2 ">
        ...
        <div class="   border-t2 clearfix ">
          <section class="blk-cont2020  " v-for="(item, index) in proposalList" :key="index">
            <div class="ek-com-tit3 bg7 f3 c4 pad1-3 border-b1"><b><img class="lazy-p" src="https://www.baoxian.com/product_file/ERiskPic/100901007.jpg" alt="pic"></b>{{item.appentName}}
<!--              <span class="f-right f2" :class="item.statustrail==='2'?'c8':'c6'">{{item.statustrail==='0'?'尚未分享':(item.statustrail==='1'?'客户未查看':(item.statustrail==='2'?'客户已查看':''))}}</span>-->
            </div>
            <ul class="cont-blk  icon-wrap ">
              <li class="li-p7 bg7 posi2 clearfix">
                <div class="product2020 f-left posi2">
                  <img class="lazy-p" v-lazy="item.ERiskPicRelaPath" alt="pic">
                </div>
                <div class="info-blk  ">
...
<!--                    <span class="f-right"><b class="fun-icon icon-edit2 "></b><b class="fun-icon icon-del "></b></span>-->
                ...
                <div class="sel-b4">
                  <label>
                    <input name="a" type="radio" :value="item.proposalId" v-model="proposalId"
                           @change="doProposal(item)"><b></b>
                  </label>
                </div>
              </li>
            </ul>
  ...
  export default {
    ...
    methods: {
      ...
      doProposal(it) {
        ...
        this.promoteLinks = {
          url: tool.app.lzgUrl + 'wx/channelShareProposal-proposalId=' + this.proposalId + '&otherPath=fzg&platformId=' + tool.app.platformId + '&platformCode=' + tool.app.platformCode,
          title: it.riskName || '',
          desc: '保障方案~',
          imageUrl: it.ERiskPicRelaPath || '',
          proposalId: it.proposalId,
          type: 2
        }
        if (this.proposalPath !== '2') {
          window.location.href = tool.app.lzgUrl + '#/source/channelUserId=' + channelUserId + '&opType=6&status=' + identificationStatus + '&proposalId=' + this.proposalId + '&platformId=' + tool.app.platformId + '&platformCode=' + tool.app.platformCode
        }
      },
      goMessBack() {
        if (Vd.required(this.proposalId, '请选择计划书')) {
          ...
          let params = {
            ...
            'shareEvent_code': 'proposalShare',//事件编码
            'shareUrl': this.promoteLinks.url,
            ...
            'share_businessId': this.promoteLinks.proposalId,//分享内容编码
            'share_businessName': this.promoteLinks.title //分享内容名称
          }
          let paramsJson = tool.cloneObject(params)
          delete paramsJson.shareUrl  //有url会出错
          this.$logVisit.putShareInfo(params) // 日志插入
          this.promoteLinks.url = this.promoteLinks.url + '-' + JSON.stringify(paramsJson)
          DataSource.set('sendShareMess', this.promoteLinks, 2)
          this.$router.go(-1)
        }
      },
    },
