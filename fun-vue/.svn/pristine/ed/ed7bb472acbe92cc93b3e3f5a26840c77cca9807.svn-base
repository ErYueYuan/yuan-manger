      ...
      <div class="my-n-txtbox ">
        ...
        >
          ...
          <div class="img-box posi2">
            <img :src="headImg ? headImg : require('@/assets/img/index/bar_bg.png')" alt="pic"/>
      ...
      <section class="pad1-3">
        <div class="bg7  b-sh1 new-nav-box tools">
          <ul class="clearfix">
            ...
            <li
              class="f2 pad1"
              @click="$router.push({ name: 'productList', params: {type: 0} })"
            >
              <span class="four"></span>产品
            </li>
            <li
              class=" f2 pad1"
              @click="goVarietyPath('1')"
            >
...
export default {
  ...
  methods: {
    ...
    goVarietyPath(type) {
      const backPath = tool.strToHexCharCode(window.location.href),
        channelUserId = this.user.userInfo.mobile || this.user.userInfo.agentcode,
        identificationStatus = this.user.userInfo.status || ''
      window.location.href = tool.app.lzgUrl + '#/source/channelUserId=' + channelUserId + '&opType=' + type + '&status=' + identificationStatus + '&platformId=' + tool.app.platformId + '&platformCode=' + tool.app.platformCode + '&backPath=' + backPath
    },
    goCard() {
      const channelUserId = this.user.userInfo.mobile || this.user.userInfo.agentcode,
        identificationStatus = this.user.userInfo.status || ''
      window.location.href = tool.app.lzgUrl + '#/source/channelUserId=' + channelUserId + '&opType=3&status=' + identificationStatus + '&platformId=' + tool.app.platformId + '&platformCode=' + tool.app.platformCode + '&backPath=' + backPath
    },
    goShop() {
