      ...
      <div class="login-box clearfix">
        ...
        <section class="login-info-box reg">
          ...
          <div class="login-input posi2">
            ...
            <b class="ek-btn-4 po1 c-p"  v-show="!sendBtn" @click="getCheckCode">获取验证码</b>
            <b class="ek-btn-88 c-p" v-show="sendBtn">{{sendTime}}</b>
          </div>
          <div class="login-input posi2">
    ...
    methods: {
      handleRegister () {
        if ( Vd.required(this.registerCredential.mobile, '请输入注册手机号')
          ...
          //   sceneCode: '1002035', //场景Code
          //   mobile: this.registerCredential.mobile,
          //   verufucationCode: this.registerCredential.checkCode
          // }
          // api_user.verifySMSCode(_data).then(data => {
          //   if (data.status == rtStatus.success) {
            ...
            // } else {
            //   tool.toastMessage(data.message || '验证码验证服务异常', 'error')
            // }
          // })
        }
      },
      getCheckCode () {
        if ( Vd.required(this.registerCredential.mobile, '请输入注册手机号')
          ...
          const _data = {
            saasId: tool.app.saasId,
            platformCode: tool.app.platformCode,
            sceneCode: '1002035', //场景Code（写死）
            mobile: this.registerCredential.mobile,
            producerCode: 0,  //验证码生成方式0-系统生成,1-调用方生成
            codeLength: 6,  //验证码长度producerCode=0时填写
            periodOfValidity: 120 //有效期
          }
          api_user.smsCode(_data).then(data => {
            if (data.status == tool.rtCode.status) {
              tool.toastMessage('短信发送成功，请注意查收')
              this.sendBtn = true
              timer = setInterval(() => {
                this.sendTime -= 1
                if (this.sendTime == 0) {
              ...
              tool.toastMessage(data.message || '服务异常', 'error')
            }
          })
        }
        this.getCount += 1
