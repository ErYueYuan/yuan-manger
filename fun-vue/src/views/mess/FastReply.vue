    ...
    <div v-show="!editStatus">
      <div class="main-container mar1  bot1 clearfix">
        <van-collapse v-model="activeName" accordion>
          <van-collapse-item class="mar3-2 posi2" v-for="(item, index) in replyList" :key="index" :name="index">
            <template #title>
              <div>{{item.categoryName}} </div><van-icon style="position: absolute;right: 50px;top: 16px;" name="cross" @click="handleDel(item)" />
            </template>
            <div class=" pad4-1" v-for="(cont, ind) in item.replyContent" :key="'cont-' + ind" :class="item.replyContent.length-1 !== ind ? 'border-b1' : ''">
              <div v-if="cont.content">
                <p class="f2 c5" v-html="cont.content"></p>
                <div class="t-right mar3">
                  <button class="ek-btn-3 min-w f1" @click="editContent(item, cont)">编辑</button>
                  <button class="ek-btn-4 min-w f1 mar5-2" @click="sendReply(cont)" v-if="!ty">发送</button>
                </div>
              </div>
            </div>
          </van-collapse-item>
    ...
    <div v-show="editStatus">
      <div class="main-container mar1  bot1 clearfix">
        ...
        <section class=" pad6-1 bg7">
          ...
          <section class="mar6 clearfix posi2">
            <div id="context" class="cont-text" contenteditable="true"></div>
          </section>
        </section>
      </div>
      ...
      <div class="mask-layer" v-show="inputMask"></div>
      <div class="tab-diy bg7" v-show="isNew" style="z-index: 24;">
        <div class="cont">
          <div class="input-box flex-center">
            <input type="text" class="ca-input3 t-right" maxlength="8" placeholder="请输入分组名称" v-model="newGroupMessage" @input="selfInput('input',$event.target.value)" >
...
<script>
  ...
  import { Collapse, CollapseItem, Empty, ActionSheet, Icon, Dialog } from 'vant'
  import { api_mess } from '@/api/index'
  import * as tool from '@/common/Tool'
  import Vd from '@/common/Validator'

  Vue.use(Collapse).use(CollapseItem).use(Empty).use(ActionSheet).use(Icon).use(Dialog)
  ...
  export default {
    ...
    methods: {
      getFastReplyList (type) {
        ...
        api_mess.funUserQuickContentGetList(_data).then(data => {
          if (data.status === tool.rtCode.status) {
            ...
            for (let item of this.replyList) {
              if (item.replyContent) {
                for (let it of item.replyContent) {
                  it.content = it.content.replace(/客户昵称/g, '<span class="c8">客户昵称</span>')
                }
              }
            }
            if (type === 'edit') {
      ...
      editContent (item, cont) { // 编辑
        ...
        let contP = document.getElementById('context')
        contP.innerHTML = ''
        let contArr = cont.content.replace(/<span>/g, '').split('<span class="c8">')
        for (let it of contArr) {
          if (it) {
            if (it.indexOf('客户昵称</span>') !== -1 && it !== '客户昵称</span>') {
              let contTail = it.split('</span>')
              let sp = document.createElement('span')
              sp.setAttribute('class', 'c8')
              sp.setAttribute('contenteditable', false)
              sp.innerHTML = '客户昵称'
              contP.appendChild(sp)
              let spo = document.createElement('span')
              spo.innerHTML = contTail[1]
              contP.appendChild(spo)
            } else if (it === '客户昵称</span>') {
              let sp = document.createElement('span')
              sp.setAttribute('class', 'c8')
              sp.setAttribute('contenteditable', false)
              sp.innerHTML = '客户昵称'
      ...
      confirmGroupAdd () {
        if (Vd.required(this.newGroupMessage, '请输入分组名称')) {
          ...
          api_mess.funUserQuickContentAddContent(_data).then(data => {
            if (data.status === tool.rtCode.status) {
              this.groupTitle = this.newGroupMessage
      ...
      soltCustomer () {
        let cont = document.getElementById('context')
        let sp = document.createElement('span')
        sp.setAttribute('class', 'c8')
        sp.setAttribute('contenteditable', false)
        sp.innerHTML = '客户昵称'
        cont.appendChild(sp)
        // let cont = document.getElementById('context').innerHTML
        // this.content = cont + `<span class="c8" contenteditable="false">客户昵称</span>`
      },
      handleEditSave () {
        let cont = document.getElementById('context').innerHTML
        this.content = cont.replace(/<span class="c8" contenteditable="false">/g, '').replace(/<\/span>/g, '')
        this.groupId = 0
        if (this.addOrEdit = 'edit') {
          this.confirmGroupEdit()
        } else {
          this.confirmGroupAdd()
        }
      },
      handleDel (it) {
        Dialog.confirm({
          title: '提示',
          message: '确定删除' + it.categoryName + '这组快捷回复吗？',
        }).then(() => {
          const _data = {
            // contentId: it.categoryId,
            categoryId: it.categoryId, // 删除整组，只传categoryId
            // userId: this.user.userInfo.userId,
            // parentId: it.parentId,
            // categoryLevel: it.categoryLevel,
            // categoryName: it.categoryName
          }
          api_mess.funUserQuickContentDeleteContent(_data).then(data => {
            if (data.status === tool.rtCode.status) {
              this.getFastReplyList()
              this.getFirstLevelList()
              tool.toastMessage('删除成功')
            } else {
              tool.toastMessage(data.message, 'error')
            }
          })
...
<style scoped>
  ...
  .cont-text{background: #F5F5F5;max-height: 100px;height:100px;overflow-y:scroll;width: 100%;padding: 10px;}
</style>
