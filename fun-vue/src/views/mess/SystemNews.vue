    ...
    <div class="main-container mar3-7 clearfix">
      <section class="bg7 mar3 b-radius3">
        <div class="pad1-3" v-for="(item, index) in systemNewsList" :key="index" @click="goNewsDetail(item)">
          <h3 class="ek-com-tit40 tri c-p f2 c6 ">{{item.keyWord.title}}</h3>
          <p class="f1 c5">{{item.keyWord.content}}</p>
          <div class="t-right c4 f1 lin-h30 border-b1">{{item.modifydate | dataFormat('yyyy-MM-dd hh:mm:ss')}}</div>
        </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      goNewsDetail (it) {
        this.$router.push({name: 'systemNewsInfo', params: {id: it._id, index: it._index, type: it._type}})
